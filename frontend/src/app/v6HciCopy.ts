import type { V6Language, V6SurfaceId } from './v6ProductionUi';

export type V6HciStatus =
  | 'ready'
...
type LocalizedCopy = Record<V6Language, string>;
...
export const v6ScreenQuestionCopy: Record<V6SurfaceId, string> = {
  ...
  web_command_center: 'What needs operator attention?',
};

export const v6HciStatusCopy: Record<V6HciStatus, Record<V6Language, StatusCopy>> = {
  ready: {
    'zh-CN': {
      label: '已准备好',
      helper: '路线和备用方案已准备好。',
...
export const v6ProviderFollowUpCopy: Record<V6Language, string[]> = {
...
export const v6PlanningProgressCopy: Record<
  V6Language,
  {
    titlePrefix: string;
    failedTitle: string;
...
> = {
  'zh-CN': {
    titlePrefix: '正在构建第一版可用行程',
    failedTitle: '生成没有完成',
    retryHint: '你的旅行需求已保留，可以调整后重试。',
  },
  en: {
    titlePrefix: 'Building the first usable itinerary',
    failedTitle: 'Generation did not finish',
    retryHint: 'Your trip request is saved. Adjust it and try again.',
  },
};

export function getV6ScreenQuestion(surface: V6SurfaceId) {
  return v6ScreenQuestionCopy[surface];
}

export function getV6HciStatusCopy(status: V6HciStatus, language: V6Language) {
  return v6HciStatusCopy[status][language];
}

export function getV6RecoveryCopy(key: keyof typeof v6RecoveryCopy, language: V6Language) {
