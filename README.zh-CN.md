...
如果你的后端平台从源码部署，而不是直接使用 Docker，建议使用 no-sync
启动命令，避免平台在运行时重新安装依赖或编译字节码：
...
- `food_specialties`：结构化本地美食和土特产资料，用于让行程更像真实旅行社顾问方案。
...
它记录了 5A 景区、省级 4A/3A 景区、全国重点文物保护单位、文旅部非遗美食线路、农产品地理标志/地方特产等数据集的官方或高权威来源候选。
...
当前结构化 manifest 是小型种子文件，用于验证管线。生产扩展应批量导入官方 5A/4A 景区列表、精选 3A 景区、全国重点文物保护单位、地方美食资料和地方特产目录。
