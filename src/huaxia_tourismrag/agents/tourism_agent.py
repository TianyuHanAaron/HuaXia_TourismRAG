...
- 不要把一天压缩成单个全天 activity；非纯休息日每天至少 3 个 activity，深度行程建议 4-6 个 activity。
- 转场日也要拆成出发交通、午餐或中途休息、抵达入住/晚餐；石窟、博物馆、徒步等半日深度体验也要把讲解/午餐/转场/休息分成独立 activity。
- 不要因为航班或火车精确时刻未知就省略所有 start_time；可以用“08:30 出发”“12:30 午餐”“15:00 入住/补给”“19:30 晚餐”等行程规划锚点，只有具体班次号和真实发车时刻需要待核验。
- 生成 12 天以上长线深度行程时，宁可压缩每条 description，也不要减少 activity 数量；前端时间线依赖多个 activity 节点展示行程节奏。
- 如果长线深度行程的输出长度有压力，优先保证 generated_itinerary 的多时间节点和 start_time 完整性；topic_sections 可以简洁，但不能牺牲每日 activity 数量。
- 每一天的午餐和晚餐都要尽量写入不同的本地美食、用餐区域或食物场景，例如夜市、美食街、老店、农家乐、酒店内清淡餐、景区外顺路餐点；尽量不要每天重复同一种小吃或同一个区域。
