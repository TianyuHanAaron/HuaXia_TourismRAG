# Step 20: Planning Loading And Progress States

## Goal
Redesign planning progress so users understand what is happening and what is preserved.

- Execution detail: turn this step into an approved UI specification for Planning Loading And Progress States before any implementation begins.
...
## User Decision Gate
Approve before implementation: loading layout default uses progress timeline plus preserved user input; copy default uses human status such as "Checking routes" and "Building your checklist"; skeleton default mirrors final layout; cancellation default is visible; retry default is explicit; motion default is calm and not flashy.
...
## Reference Inputs
Use V7 SSE coverage, V6 progressive data, and modern loading patterns.
...
## Product Behavior
Users see progress, partial results, and recovery options during trip generation.
...
## Web UI Scope
Web shows richer job progress and citations.
...
## Mobile UI Scope
Mobile shows concise progress and next safe action.
...
## Data Flow
SSE events and job status map into progress states.
...
## Edge Cases
Slow jobs, disconnected SSE, retry, cancellation, and partial draft availability.
...
## Test Plan
Test progress events, reconnect, error, retry, and partial result display.
...
## Acceptance Criteria
No silent loading or unexplained spinner remains in planning.
...
## Dependencies
Depends on Steps 17 through 19.
