# Step 23: Trip Home Command Center

## Goal
Redesign Trip Home as the primary daily travel command surface.

- Execution detail: turn this step into an approved UI specification for Trip Home Command Center before any implementation begins.
...
## User Decision Gate
Approve before implementation: first screen default shows active trip, current phase, next best action, today task count, and one risk/reminder card; layout default uses destination/map hero plus compact command stack; CTA default is one primary action; density default avoids dashboard clutter; imagery default is trip photo or map; screen states default include cached, syncing, offline, empty, and urgent.
...
## Reference Inputs
Use FocusFlight home execution, Wanderlog trip overview, and V6 Trip Home requirements.
...
## Product Behavior
Trip Home answers "what should I do next?" within two seconds.
...
## Web UI Scope
Web command center mirrors summary and operational status.
...
## Mobile UI Scope
Mobile Trip Home is the flagship surface.
...
## Data Flow
Trip summary, current phase, next action, risk, sync status, and cached data feed the view.
...
## Edge Cases
No active trip, stale cache, offline, delayed summary, and urgent departure state.
...
## Test Plan
Test cached render, server reconcile, large text, offline, and urgent states.
...
## Acceptance Criteria
Trip Home is beautiful, fast, and action-focused.
