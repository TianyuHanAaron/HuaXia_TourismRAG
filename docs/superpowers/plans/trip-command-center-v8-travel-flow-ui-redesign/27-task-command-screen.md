# Step 27: Task Command Screen

## Goal
Redesign the task screen as a command list for travel execution.

- Execution detail: turn this step into an approved UI specification for Task Command Screen before any implementation begins.
...
## User Decision Gate
Approve before implementation: groups default to Now, Today, Upcoming, Blocked, Completed; card default shows title, due time, phase chip, priority, instruction, sync state, and primary action; swipe default is right complete, left skip/edit; density default is compact; copy default says what to do; visual default uses status chips and icon-led rows.
...
## Reference Inputs
Use BlaBlaCar trust flows, Wanderlog lists, and V4 task screen requirements.
...
## Product Behavior
Tasks answer "what needs action now?"
...
## Web UI Scope
Web task list supports review and admin debugging.
...
## Mobile UI Scope
Mobile task command supports fast completion and recovery.
...
## Data Flow
Task groups, phase, priority, dependency, and provider status feed card variants.
...
## Edge Cases
Blocked, overdue, offline completed, conflict, skipped, and restored tasks.
...
## Test Plan
Test grouping, complete, skip, edit, blocked reason, and offline sync.
...
## Acceptance Criteria
Task action feels instant, clear, and reversible.
