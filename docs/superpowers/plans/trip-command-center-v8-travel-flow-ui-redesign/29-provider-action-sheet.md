# Step 29: Provider Action Sheet

## Goal
Redesign provider action sheets for confident external handoff.

- Execution detail: turn this step into an approved UI specification for Provider Action Sheet before any implementation begins.
...
## User Decision Gate
Approve before implementation: sheet style default is FocusFlight dark glass for execution; content default shows provider, destination, route/search summary, confidence, fallback, and validation state; primary action default hides when invalid; alternatives default are secondary; follow-up default offers completed, remind me later, and something went wrong.
...
## Reference Inputs
Use FocusFlight dark execution surfaces, BlaBlaCar handoff trust, and V3 provider plans.
...
## Product Behavior
Provider Sheet answers "where will I go if I tap this?"
...
## Web UI Scope
Web can preview provider context but does not mimic native launch behavior.
...
## Mobile UI Scope
Mobile owns the full handoff and follow-up interaction.
...
## Data Flow
Validated provider action, route bundle, fallback URL, and audit state feed the sheet.
...
## Edge Cases
Invalid route, missing destination, provider unavailable, offline, and launch failure.
...
## Test Plan
Test valid, invalid, fallback, launch, follow-up, and screen-reader labels.
...
## Acceptance Criteria
Broken actions never render as primary CTAs.
...
## Dependencies
Depends on Steps 24, 27, and V3 provider validation.
