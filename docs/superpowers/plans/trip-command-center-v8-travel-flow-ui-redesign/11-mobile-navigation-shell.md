# Step 11: Mobile Navigation Shell

## Goal
Redesign the mobile navigation shell for fast trip execution.

- Execution detail: turn this step into an approved UI specification for Mobile Navigation Shell before any implementation begins.
...
## User Decision Gate
Approve before implementation: tab labels default to Home, Timeline, Tasks, Documents, Settings; tab style default is compact icon plus label; active state default uses route blue underline and filled icon; shell default respects safe area; modal entry default uses bottom sheets; density default keeps one-handed access; screen states default include cached, loading, offline, and blocked routes.
...
## Reference Inputs
Use Wanderlog tabs, FocusFlight bottom surfaces, and V4 mobile stack IA.
...
## Product Behavior
The shell keeps travelers oriented without crowding the task surface.
...
## Web UI Scope
No web change except shared naming consistency.
...
## Mobile UI Scope
Update navigation spec for Expo Router and bottom tabs.
...
## Data Flow
Selected trip, selected tab, and open sheet are UI state only.
...
## Edge Cases
If no active trip exists, Home routes to onboarding and sample trip states.
...
## Test Plan
Test tabs, modal entry, safe areas, large text, and offline state.
...
## Acceptance Criteria
Navigation is predictable, thumb-friendly, and visually aligned with V8.
