# Step 9: Iconography Imagery And Map Visuals

## Goal
Define icon, photo, illustration, and map preview treatment for V8.

- Execution detail: turn this step into an approved UI specification for Iconography Imagery And Map Visuals before any implementation begins.
...
## User Decision Gate
Approve before implementation: icon default is filled or strong-stroke travel glyphs; map default uses contextual preview, not decorative background; imagery default uses real destination or generated travel context; empty-state illustration default is small and purposeful; provider states default use recognizable symbols; decorative blobs default are excluded.
...
## Reference Inputs
Use FocusFlight map atmosphere, Wanderlog map placement, BlaBlaCar icon clarity, and Marriott restrained media.
...
## Product Behavior
Visuals help recognition, orientation, and action confidence.
...
## Web UI Scope
Web uses map and image previews where they support planning review.
...
## Mobile UI Scope
Mobile uses imagery sparingly and prioritizes route context.
...
## Data Flow
Route, place, provider, document, and task type select icon and visual treatment.
...
## Edge Cases
If imagery is unavailable, use a clean contextual map or icon-led state.
...
## Test Plan
Check icon alignment, meaning, state color, and screen-reader labels.
...
## Acceptance Criteria
Every icon has a clear meaning and consistent optical weight.
