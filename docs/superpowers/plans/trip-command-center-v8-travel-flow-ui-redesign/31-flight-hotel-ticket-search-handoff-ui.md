# Step 31: Flight Hotel Ticket Search Handoff UI

## Goal
Design search and handoff surfaces for flights, hotels, and activity tickets.

- Execution detail: turn this step into an approved UI specification for Flight Hotel Ticket Search Handoff UI before any implementation begins.
...
## User Decision Gate
Approve before implementation: layout default shows search context before provider launch; card default shows dates, travelers, location, price context, provider, confidence, and fallback; booking default remains external handoff; CTA default is Search on provider; visual default uses Marriott clarity with FocusFlight execution accents; screen states default include incomplete, ready, unavailable, and handed off.
...
## Reference Inputs
Use Marriott booking review, Wanderlog trip structure, and V3 provider search/handoff rules.
...
## Product Behavior
Users understand what search context will open externally.
...
## Web UI Scope
Web supports richer comparison and provider debug detail.
...
## Mobile UI Scope
Mobile supports compact search context and provider launch.
...
## Data Flow
Trip dates, travelers, destination, task, and provider registry feed handoff UI.
...
## Edge Cases
Missing dates, uncertain travelers, provider unavailable, and unsupported region.
...
## Test Plan
Test flight, hotel, ticket, incomplete search, fallback, and launch follow-up.
...
## Acceptance Criteria
V8 does not imply in-app booking where only handoff exists.
