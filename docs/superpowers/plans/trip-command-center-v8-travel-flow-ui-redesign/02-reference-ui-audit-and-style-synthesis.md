# Step 2: Reference UI Audit And Style Synthesis

## Goal
Turn the five UI reference packs into a single actionable visual direction.

- Execution detail: turn this step into an approved UI specification for Reference UI Audit And Style Synthesis before any implementation begins.
...
## User Decision Gate
Approve before implementation: primary anchor default is FocusFlight execution polish; structure default is Wanderlog; timeline default is Timepage; trust flow default is BlaBlaCar; booking clarity default is Marriott; rejected default is copying any reference wholesale; synthesis default is Immersive Command.
...
## Reference Inputs
Use `/UI/FocusFlight ios Apr 2026`, `/UI/Wanderlog ios Sep 2025`, `/UI/Timepage ios Aug 2023`, `/UI/BlaBlaCar ios May 2026`, and `/UI/Marriott Bonvoy ios Jul 2025`.
...
## Product Behavior
The audit explains which reference owns each product moment.
...
## Web UI Scope
Web borrows Marriott clarity and Wanderlog structure, with restrained FocusFlight accents.
...
## Mobile UI Scope
Mobile uses FocusFlight for execution, Timepage for timeline, and BlaBlaCar for actions.
...
## Data Flow
Reference choices map to `VisualConceptBrief` and `ComponentVariantSpec`.
...
## Edge Cases
If a reference conflicts with accessibility, accessibility wins.
...
## Test Plan
Review the audit against all five reference folders.
...
## Acceptance Criteria
Each reference has a clear role and a clear non-goal.
...
## Dependencies
Depends on Step 1 approval protocol.
