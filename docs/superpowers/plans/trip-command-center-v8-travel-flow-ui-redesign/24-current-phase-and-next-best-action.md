# Step 24: Current Phase And Next Best Action

## Goal
Design the current phase module and next-best-action card with phase-aware urgency.

- Execution detail: turn this step into an approved UI specification for Current Phase And Next Best Action before any implementation begins.
...
## User Decision Gate
Approve before implementation: phase chip default shows phase, urgency, and sync state; next action default shows title, why now, due time, provider readiness, and primary CTA; blocked default shows one reason and unlock task; color default follows mood; copy default starts with the action; motion default highlights changes without pulsing.
...
## Reference Inputs
Use FocusFlight execution confidence and BlaBlaCar action clarity.
...
## Product Behavior
Users can act without remembering itinerary details.
...
## Web UI Scope
Web shows the same next action in command center.
...
## Mobile UI Scope
Mobile card gets priority placement on Trip Home.
...
## Data Flow
Phase, task, provider validation, and risk data produce the card state.
...
## Edge Cases
No actionable task, blocked task, overdue action, invalid provider, and offline completion.
...
## Test Plan
Test ready, blocked, overdue, invalid, and offline states.
...
## Acceptance Criteria
The card makes the next action unmistakable and recoverable.
