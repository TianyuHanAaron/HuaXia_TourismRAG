# Step 10: Motion Feedback And Microinteractions

## Goal
Define motion rules that make the UI feel modern without adding distraction.

- Execution detail: turn this step into an approved UI specification for Motion Feedback And Microinteractions before any implementation begins.
...
## User Decision Gate
Approve before implementation: motion default is subtle press feedback, bottom-sheet spring, optimistic task completion, route-preview reveal, skeleton shimmer, and conflict sheet focus; reduced-motion default keeps state changes visible without animation; toast default is brief and action-specific; loading default shows progress and preserved data.
...
## Reference Inputs
Use FocusFlight execution feel, Timepage transitions, and V6 motion feedback requirements.
...
## Product Behavior
Motion clarifies status, hierarchy, and completion.
...
## Web UI Scope
Web motion supports planning progress and admin state transitions.
...
## Mobile UI Scope
Mobile motion supports taps, swipes, sheets, sync, and handoff follow-up.
...
## Data Flow
State transitions trigger motion variants from `MotionFeedbackSpec`.
...
## Edge Cases
Motion must not hide errors, provider failures, or offline conflicts.
...
## Test Plan
Test normal motion and reduced-motion settings.
...
## Acceptance Criteria
Every animated state has an accessible non-motion signal.
