# Step 38: Empty Error Loading And Recovery States

## Goal
Define the full state system for empty, loading, error, blocked, and recovery moments.

- Execution detail: turn this step into an approved UI specification for Empty Error Loading And Recovery States before any implementation begins.
...
## User Decision Gate
Approve before implementation: empty states default show one action and one explanation; errors default explain what happened, what to do, and what is safe; loading default uses skeleton matching final layout; blocked default shows unlock action; retry default is visible; illustration default is small and purposeful.
...
## Reference Inputs
Use BlaBlaCar plain language, Marriott recoverability, and V6 HCI requirements.
...
## Product Behavior
Users never face silent failure or vague system language.
...
## Web UI Scope
Web errors include support and admin detail when appropriate.
...
## Mobile UI Scope
Mobile errors prioritize recovery and preserved data.
...
## Data Flow
Network, validation, provider, cache, and task states select the message variant.
...
## Edge Cases
No trip, no tasks, network failure, provider invalid, document failure, and auth expiry.
...
## Test Plan
Test every major screen in empty, loading, error, and recovery states.
...
## Acceptance Criteria
Every failure offers a clear next step.
...
## Dependencies
Depends on Step 1 decision protocol.
