# Step 1: User Decision Gate Protocol

## Goal
Define how every UI choice is paused, presented, approved, and recorded before implementation.

- Execution detail: turn this step into an approved UI specification for User Decision Gate Protocol before any implementation begins.
...
## User Decision Gate
Approve before implementation: gate format default is concrete options with one recommended default; approval record default stores date, screen, decision, selected option, rejected options, and implementation notes; unresolved decision default blocks work; visual changes default require screenshot or concept evidence; copy changes default require exact approved wording.
...
## Reference Inputs
Use the user's explicit request for fine-grained control and the V8 folder contract.
...
## Product Behavior
Design decisions become deliberate and traceable rather than inferred by implementers.
...
## Backend Scope
No backend changes; `UiApprovalRecord` is documentation-only.
...
## Web UI Scope
Web decisions use the same approval format as mobile.
...
## Mobile UI Scope
Mobile decisions include safe area, tap target, density, gesture, and native handoff choices.
...
## Data Flow
Approved decisions feed concept briefs, component specs, and implementation tasks.
...
## Edge Cases
If the user changes a decision, later steps must reference the newest approval.
...
## Test Plan
Check every numbered file includes a concrete approval gate.
...
## Acceptance Criteria
No step permits silent visual decisions.
...
## Dependencies
Depends on README decision protocol.
