# Step 37: Offline Sync And Conflict Resolution UI

## Goal
Redesign offline, sync, and conflict states so they feel reliable.

- Execution detail: turn this step into an approved UI specification for Offline Sync And Conflict Resolution UI before any implementation begins.
...
## User Decision Gate
Approve before implementation: offline banner default is subtle and persistent; task states default are Saved locally, Syncing, Synced, and Conflict; conflict UI default is focused bottom sheet; copy default says what the app kept safe; retry default is explicit; color default uses jade for synced and amber for review.
...
## Reference Inputs
Use V4 offline UX and V6 recovery requirements.
...
## Product Behavior
Offline completion feels instant and reconciles later.
...
## Web UI Scope
Web shows offline state only where supported.
...
## Mobile UI Scope
Mobile owns offline queue, banner, and conflict resolution UX.
...
## Data Flow
Offline queue, cached trip, mutation state, and server result drive sync indicators.
...
## Edge Cases
Repeated failure, changed task, deleted task, duplicate completion, and stale cache.
...
## Test Plan
Test offline complete, reconnect, sync success, conflict, retry, and dismiss.
...
## Acceptance Criteria
Offline states are visible, calm, and actionable.
