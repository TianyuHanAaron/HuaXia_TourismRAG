# Step 30: Route Preview Map And Handoff

## Goal
Design route preview, map context, and navigation handoff interactions.

- Execution detail: turn this step into an approved UI specification for Route Preview Map And Handoff before any implementation begins.
...
## User Decision Gate
Approve before implementation: preview default shows origin, destination, travel mode, provider, duration, distance, confidence, and fallback; map style default is contextual and uncluttered; launch copy default is Open route; invalid copy default says what route detail is missing; alternatives default include provider choices and manual copy.
...
## Reference Inputs
Use FocusFlight map mood, Wanderlog map add-to-plan, and V3 route bundle requirements.
...
## Product Behavior
Users see route context before leaving the app.
...
## Web UI Scope
Web route preview is informational and supports copy/open link.
...
## Mobile UI Scope
Mobile route preview prepares native map handoff.
...
## Data Flow
Route bundle and provider validation drive preview states.
...
## Edge Cases
Missing origin, missing destination, low confidence, region-specific provider, and offline.
...
## Test Plan
Test ready route, missing fields, fallback URL, provider switch, and large text.
...
## Acceptance Criteria
Navigation handoff is never empty or surprising.
