# Step 48: Responsive Accessibility Performance QA

## Goal
Define QA gates for responsive layout, accessibility, dynamic type, and rendering performance.

- Execution detail: turn this step into an approved UI specification for Responsive Accessibility Performance QA before any implementation begins.
...
## User Decision Gate
Approve before implementation: viewport matrix default covers small phone, large phone, tablet, small desktop, and wide desktop; accessibility default includes 44px targets, dynamic text, screen-reader labels, color-independent status, and reduced motion; performance default uses virtualized lists, cached trip render, skeletons, and no layout jumps.
...
## Reference Inputs
Use V6 QA steps, V7 E2E plans, and frontend/mobile testing coverage.
...
## Product Behavior
The redesign remains usable under real device and accessibility constraints.
...
## Web UI Scope
Web must pass responsive, keyboard, screen-reader, and build checks.
...
## Mobile UI Scope
Mobile must pass safe area, dynamic text, offline, and native flow checks.
...
## Data Flow
QA fixtures cover active trip, long trip, offline, provider invalid, documents, and safety.
...
## Edge Cases
Large text, narrow device, landscape, slow network, stale cache, and long lists.
...
## Test Plan
Run unit, typecheck, Playwright web, Expo web, Maestro native, and screenshot QA where available.
...
## Acceptance Criteria
No redesigned surface ships without accessibility and responsive evidence.
